...
def get_features(params,pca=None,scaler=None):
    ...
    for image_name in image_list:
        ...
        #f_sift = image_local_features(im)
        #f_sift=sift_features(im)

        # posem f_i que sigui igual que la dimensi f_o
        #f_sift.resize(f_sift.shape)
        #feats=np.concatenate((f_sift,f_sift))

        feats = image_local_features(params, im)
        #f_sift.resize(f_root.shape)
        #feats=np.concatenate((f_root,f_sift))
...
def image_local_features(params, image):
    ...
    kp = sift.detect(image,None) #Detecta los keypoints
    kp,des = sift.compute(gray, kp, params['descriptor_size'])
    return des
