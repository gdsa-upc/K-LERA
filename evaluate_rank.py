...
def display(params,query_id,ranking,relnotrel):
    ...
    # Display
    ax.imshow(query_im)
    ax.axes.get_xaxis().set_visible(False) #Elimina los ejes horizontales
    ax.axes.get_yaxis().set_visible(False) #Elimina los ejes verticales
    ...
    for i in range(15):
        ...
        #Así tenemos las imágenes que coinciden en verde y las que no en rojo
...
def get_hitandmiss(ranking,query_class,annotation_train):
    ...
    for i in ranking[0].tolist():
        ...
        i_class = list(annotation_train.loc[annotation_train['ImageID'] == i]['ClassID'])[0] #lista las clases de las imágenes

        # And if it matches the query class...
        if query_class == i_class: #compara si coincide con la clase

            # Then it means it's correct
            relnotrel.append(1) #añade true si tienen la misma clase
        else:
...
print np.mean(ap_list) #Mediana aritmŽtica